...
import { createHttpSession, getCookieForSite, htmlToText, pageTitleFromHtml, readText } from "./http-session.mjs";
import { isNaixiAlreadySigned } from "./naixi-sign.mjs";
...
/**
 * 还原 Discuz 的 inajax 响应。
 *
 * 带 inajax=1 的请求返回的是 XML+CDATA，正文往往只有一段
 * `<script>showWindow('qiandao', '<div class="alert_right">恭喜你签到成功…</div>')</script>`。
 * htmlToText 会把 <script> 整段删掉，CDATA 包装又被当成标签吃掉，
 * 最后只剩下末尾裸露的 "]]>"，于是签到明明成功了却被判成失败。
 *
 * 这里先脱掉 CDATA，再把 script 里的字符串字面量捞出来附在后面，
 * 让后续的关键词判断能看到真正的提示文案。
 */
export function unwrapDiscuzAjax(html = "") {
  const source = String(html || "");
  const cdata = source.match(/<!\[CDATA\[([\s\S]*?)\]\]>/i);
  if (!cdata) return source;
  const inner = cdata[1];
  if (!/<script/i.test(inner)) return inner;

  const literals = [];
  for (const block of inner.matchAll(/<script[^>]*>([\s\S]*?)<\/script>/gi)) {
    for (const quoted of block[1].matchAll(/'((?:[^'\\]|\\.)*)'|"((?:[^"\\]|\\.)*)"/g)) {
      const value = (quoted[1] ?? quoted[2] ?? "").replace(/\\(['"\\])/g, "$1");
      // 只保留像提示文案的字符串，URL / 选择器 / 单词参数没有意义。
      if (value && !/^[\w.\-/?=&#:]*$/.test(value)) literals.push(value);
    }
  }
  return literals.length ? `${inner}\n${literals.join("\n")}` : inner;
}

async function openText(session, path, steps, label, baseForTitle = "") {
  const res = await session.get(path);
  const html = unwrapDiscuzAjax(await readText(res));
...
async function runNaixi(siteConfig, secrets) {
  ...
  // 提交响应是 ajax 片段，格式随插件版本变。以签到页的实际状态为准复查一次，
  // 这样即使响应解析不出文案，只要签上了就仍然算成功（其他 Discuz 站点也都有这一步）。
  const verify = await openText(session, `${origin}/k_misign-sign.html`, steps, "HTTP 复查奶昔签到状态");
  stats = parseNaixiStats(`${signed.text} ${verify.text} ${page.text}`, verify.html || signed.html || page.html);
  const ok = signed.res.status < 400
    && (/签到成功|恭喜|获得奖励|已签到|今日已签|今天已签|您今天已经签到/.test(signed.text)
      || isNaixiAlreadySigned(verify.text));
  return { success: ok, message: ok ? `签到成功${stats.reward ? `，奖励 ${stats.reward}` : ""}${Number.isFinite(stats.streakDays) ? `；连续签到 ${stats.streakDays} 天` : ""}；签到时间：${signTime}` : `签到失败：${compactText(`${signed.text} ${verify.text}`).slice(0, 160) || "签到响应无法解析"}`, details: { signTime, clickedSignIn: true, checkinAction: ok ? "api_signed" : "api_failed", ...stats, pageTitle: verify.title || signed.title || page.title }, steps };
}
