...
import { importPlaywright, launchBrowser, resolveChromiumExecutablePath  } from "../utils/browser.mjs";
import { wantsHttpMode, allowsHttpFallback, runSiteHttp } from "../utils/site-http.mjs";
import { absoluteV2EXRedeemUrl, findV2EXDailyRedeemHref } from "./v2ex-utils.mjs";
...
export default class V2EXDriver extends BaseDriver {
  ...
  async signIn() {
    ...
    try {
      ...
      // page.goto 必须是绝对 URL；V2EX 给的是 /mission/daily/redeem?once=xxx 这种相对路径。
      const redeemUrl = absoluteV2EXRedeemUrl(redeemHref, origin);
      const redeem = await page.goto(redeemUrl, { waitUntil: "domcontentloaded", timeout });
      ...
      return {
        ...
        steps: [
          ...
          { label: "注入 Cookie 并准备浏览器上下文", ok: true },
          { label: "打开 V2EX 每日任务页面", ok: true, status: daily?.status() || null },
          { label: "找到并校验官方领取奖励链接", ok: true, detail: redeemUrl },
